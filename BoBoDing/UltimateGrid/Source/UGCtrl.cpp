...
BEGIN_MESSAGE_MAP(CUGCtrl, CWnd)
	...
	///********************************************************************/
	//ON_EN_CHANGE(UG_EDITID, OnChangeEdit)  //标记着消息映射 增加 邵盛松
	//ON_EN_KILLFOCUS(UG_EDITID, OnKillFocusEdit) //编辑框失去焦点 2011-6-27
	///********************************************************************/

	//ON_CONTROL_REFLECT(EN_CHANGE, OnChangeEdit)
	//ON_CONTROL_REFLECT(UG_EDITID, &CUGCtrl::OnChangeEdit)  //标记着消息映射 增加 邵盛松
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CUGCtrl::CreateChildWindows(){
	...
	//{m_pDlg=new CDlgXiaoShouDanPrompt;
	//m_pDlg->Create(IDD_DIALOG_XIAOSHOUDAN_PROMPT,this);
	//m_pDlg->m_pUG=this;
...
int	CUGCtrl::StartEdit(int col,long row,int key)
{
	...
	if(m_GI->m_defDataSource->OnEditStart(m_editCol,m_editRow,&m_editCtrl) != FALSE)
	{
		...
		{	
			...
			/***********************************************************************/
			//2011-5-27 修改 作者：邵盛松 ShengSong Shao
			m_editCtrl->SetWindowPos(&wndTop,rect.left+3,rect.top+1,
				rect.right-rect.left-3,rect.bottom-rect.top-1,0);
			//宽度为25
			/***********************************************************************/
			...
			////add bu BuGu
			//CString strTemp=m_editCell.GetText();
			//int nLength=strTemp.GetLength();
			//((CEdit *)this->m_editCtrl)->SetSel(nLength,-1);//CUGEdit
...
//void CUGCtrl::OnChangeEdit()
//{
//	//2011-6-26
//	 //如果在CUGEditBase::OnEnChange()中响应函数，那么CUGCtrl::OnChangeEdit()函数将不会响应
//	if (GetCurrentCol()==1)//2011-6-29 在品名列中提示
//	{
//		CString strXiaoShouDan=_T("");
//		CString strTemp;
//		m_defEditCtrl.GetWindowText(strTemp);
//		strXiaoShouDan=strTemp;
//		strXiaoShouDan.Replace(_T("\""),_T("\"\""));
//		strXiaoShouDan.Replace(_T("'"),_T("''"));
...
//		{	
//		
//			m_pDlg->QueryBuilder(strXiaoShouDan);
//			CRect editRect;
//			CRect windowsRect;
//			m_defEditCtrl.GetWindowRect(editRect)   ;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             
//			m_pDlg->GetWindowRect(&windowsRect);
//			//ClientToScreen(&editRect);
//
//			m_pDlg->MoveWindow(editRect.left,editRect.bottom,	windowsRect.Width(),windowsRect.Height());
//			m_pDlg->InitList();
//			m_defEditCtrl.SetFocus();
//			/****************************************************************************/
//			// 邵盛松 2011-6-27 添加
//			//功能 在单元输入内容时，弹出下拉列表，焦点禁止发生变化，用户还可以在单元格内输入
//			StartEdit();
//			int nLength=strTemp.GetLength();
//			m_defEditCtrl.SetSel(nLength,-1);
//			/****************************************************************************/
//		}
//		else
//		{
...
//	
//}
//void CUGCtrl::OnKillFocusEdit()
//{
//	//m_pDlg->ShowWindow(SW_HIDE);
//}
//void CUGCtrl::SetPromptText(CString strText)
//{
//	//QuickSetText(GetCurrentCol(),GetCurrentRow(),strText);
//	QuickSetText(GetCurrentCol(),GetCurrentRow(),strText);
//
//
//
...
//
//		if (!strText.IsEmpty())
//		{//strSQL.Format(_T("select PinMingGuiGe,DanWei,,JinHuoJia,PiFaJia,LingShouJia 
//
//			CString strSQL;
//			strSQL.Format(_T("select  DanWei from KuCun where PinMingGuiGe='%s';"),DealWithValue(strText));
//			CppSQLite3DB db;
//			db.open(CBoBoDingApp::g_strDatabasePath);
//			CppSQLite3Query q = db.execQuery(strSQL);
//			if (!q.eof())
//			{
//					QuickSetText(GetCurrentCol()+1,GetCurrentRow(),q.fieldValue(_T("DanWei")));
//			
//			}
//		}
//	}
...
//	Invalidate(TRUE);
//}
//CString CUGCtrl::DealWithValue(CString strTemp)
//{
