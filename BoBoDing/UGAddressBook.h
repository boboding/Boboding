// CUGAddressBook.h : interface of the CUGAddressBook class
//
/////////////////////////////////////////////////////////////////////////////
#include "ugctrl.h"
#include "ugctbutn.h"
//#include "../CellTypes/ugctelps.h"//省略号按钮
//#include "BuGu.h"

#define BUTTON_CLICK_CHANGE 299
#define BUTTON_CLICK_DEL 300
class CUGAddressBook:public CUGCtrl
{
public:
	CUGAddressBook();
	~CUGAddressBook();
...
public:
	...
	int m_nRows ;
	int m_nCols ;
	//long GetRowCount(COleDateTime oleDateTimeFrom,COleDateTime oleDateTimeTo,CString strKehu);
	long GetRowCount();//COleDateTime oleDateTimeFrom,COleDateTime oleDateTimeTo,CString strKehu);
	// 获取单据编号
	CString GetDanJuBianHao(int nCol,long nRow);
	CString m_strDanJuBianHao;

	void SQLQuery(COleDateTime oleDateTimeFrom,COleDateTime oleDateTimeTo,CString strKehu);
	void InitData(COleDateTime oleDateTimeFrom,COleDateTime oleDateTimeTo,CString strKehu);
	double GetHeJiXiaoShouJinE();
	double GetHeJiZheHouJinE();
	double GetHeJiShouKuanJinE();
	...
	void Query(int nSelect,CString strCondition);
	long GetRowCount(int nSelect,CString strCondition);
